...
static const int OUTPUT_CLS_SIZE = 8;
static const int OUTPUT_BBOX_SIZE = OUTPUT_CLS_SIZE * 4;

const std::string CLASSES[OUTPUT_CLS_SIZE]{ "background", "norm", "left", "right", "down", "sleep", "yawn", "phone" };
...
{
	std::string file = "data/samples/DSOD/" + input;
	struct stat info;
	int i, MAX_DEPTH = 10;
    ...
    if (i == MAX_DEPTH)
    {
		file = std::string("data/DSOD/") + input;
		for (i = 0; i < MAX_DEPTH && stat(file.c_str(), &info); i++)
			file = "../" + file;		
    }
...
public:
	...
	Dims getOutputDimensions(int index, const Dims* inputs, int nbInputDims) override
	{
		...
		std::cout << "inputs[0].d[0] " << inputs[0].d[0] << std::endl;
		std::cout << "inputs[0].d[1] " << inputs[0].d[1] << std::endl;
		std::cout << "inputs[0].d[2] " << inputs[0].d[2] << std::endl;
		/*Dims dim;
		dim.nbDims = 1;
...
public:
	...
	virtual nvinfer1::IPlugin* createPlugin(const char* layerName, const nvinfer1::Weights* weights, int nbWeights) override
	{
		...
		else if (strstr(layerName, "_flat") != nullptr)
		{
		...
		else if (!strcmp(layerName, "mbox_conf_reshape"))
		{
			...
			assert(nbWeights == 0 && weights == nullptr);
			mPluginRshp8 = std::unique_ptr<Reshape<OUTPUT_CLS_SIZE>>(new Reshape<OUTPUT_CLS_SIZE>());
			return mPluginRshp8.get();
		}
		else if  ((strstr(layerName, "_norm_mbox_loc_perm") != nullptr || strstr(layerName, "_norm_mbox_conf_perm") != nullptr))
		{
		...
		else if (strstr(layerName, "_norm_mbox_priorbox") != nullptr) 
		{
		...
		else if (!strcmp(layerName, "detection_out"))
		{
			...
			assert(nbWeights == 0 && weights == nullptr);
			mPluginDetectionOut = std::unique_ptr<DetectionOutput<OUTPUT_CLS_SIZE>>(new DetectionOutput<OUTPUT_CLS_SIZE>());
		...
		else if (strstr(layerName, "_norm") != nullptr && !strcmp( strchr(layerName, '_') , "_norm"))//(!strcmp(strchr(layerName, '_'), "_norm"))
		{
	...
	IPlugin* createPlugin(const char* layerName, const void* serialData, size_t serialLength) override
	{
		...
		{
			assert(mPluginRshp8 == nullptr);
			mPluginRshp8 = std::unique_ptr<Reshape<OUTPUT_CLS_SIZE>>(new Reshape<OUTPUT_CLS_SIZE>(serialData, serialLength));
			return mPluginRshp8.get();
		}
		else if ((strstr(layerName, "_norm_mbox_loc_perm") != nullptr || strstr(layerName, "_norm_mbox_conf_perm") != nullptr))
		{
		...
		{
			assert(mPluginDetectionOut == nullptr);
			mPluginDetectionOut = std::unique_ptr<DetectionOutput<OUTPUT_CLS_SIZE>>(new DetectionOutput<OUTPUT_CLS_SIZE>(serialData, serialLength));
			return mPluginDetectionOut.get();
		}
		/*else if (!strcmp(layerName, "conv4_3_norm"))
		{
		...
		else if (strstr(layerName, "_norm") != nullptr && !strcmp( strchr(layerName, '_') , "_norm"))
		{
	...
	bool isPlugin(const char* name) override
	{
		...
		return (strstr(name, "mbox_loc_perm") != nullptr || strstr(name, "mbox_conf_perm") != nullptr)//permute
			|| (strstr(name, "_norm") != nullptr && !strcmp( strchr(name, '_') , "_norm"))
			|| (strstr(name, "_flat") != nullptr)//flatten
			|| (strstr(name, "_mbox_priorbox") != nullptr)//priorbox
			|| !strcmp(name, "mbox_priorbox") //concat axis2
			|| !strcmp(name, "mbox_conf_reshape")//Reshape
			|| !strcmp(name, "mbox_conf_softmax")//softmax
			|| !strcmp(name, "detection_out")
			|| !strcmp(name, "printDims")
			|| (strchr(name, '_') && !strcmp(strchr(name, '_') , "_norm"));
	}
	...
	std::unique_ptr<Reshape<OUTPUT_CLS_SIZE>> mPluginRshp8{ nullptr };
	...
	std::unique_ptr<DetectionOutput<OUTPUT_CLS_SIZE>> mPluginDetectionOut{ nullptr };
...
int main(int argc, char** argv)
{
	...
	caffeToGIEModel("IPlugin.prototxt",
		"DSOD300_VOC0712_DSOD300_300x300_iter_46000.caffemodel",
	...
	// available images 
	std::vector<std::string> imageList = { "000456.ppm", "004545.ppm", "test.jpg" };
	...
	/*for (int i = 0; i < N; ++i)
	{
		readPPMFile(imageList[2], ppms[i]);
...
//layer_width_ * layer_height_ * num_priors_ * 8
	...
	//cv::VideoCapture cap("/media/nvidia/ffb00bf7-359d-4db7-9c4b-1be65e01db25/cap_2_2017-07-11_09_11_53.avi");
	cv::VideoCapture cap(1);
	cap.set(CV_CAP_PROP_FRAME_WIDTH, 533);
	...
	for(int i = 0; i < OUTPUT_CLS_SIZE; i++) lableName[i] = CLASSES[i];
	VisualizeBBox(img, out, 0.5f, GetColors(OUTPUT_CLS_SIZE), lableName, "");
